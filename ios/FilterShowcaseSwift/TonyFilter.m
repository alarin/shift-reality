...
@implementation TonyFilter

NSString *const kGPUImageTonyShaderString = SHADER_STRING
(
 ...
 void main()
 {
   ...
   verticalDerivative = verticalDerivative * 3.0;
   horizontalDerivative = horizontalDerivative * 3.0;
...
//   gl_FragColor = vec4(horizontalDerivative * horizontalDerivative, verticalDerivative * verticalDerivative, ((verticalDerivative * horizontalDerivative) + 1.0) / 2.0, 1.0);
   gl_FragColor = vec4(horizontalDerivative * horizontalDerivative, verticalDerivative * verticalDerivative, verticalDerivative * horizontalDerivative * edgeStrength, 1.0);
...
- (id)init;
{
  if (!(self = [self initWithFragmentShaderFromString:kGPUImageTonyShaderString]))
  {
    return nil;
  }
